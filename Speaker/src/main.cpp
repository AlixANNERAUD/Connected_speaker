...
bool Check_Authentification(AsyncWebServerRequest *Request, bool Refresh_Timer)
{
...
void Setup_Web_Server()
{
    ...
    Web_Server.on("/set", HTTP_POST, [](AsyncWebServerRequest *Request) {
        Serial.print(F("/set :"));
        if (Check_Authentification(Request))
        {
    ...
    Web_Server.on("/get", HTTP_POST, [](AsyncWebServerRequest *Request) {
        if (!Check_Authentification(Request))
        {
            ...
            else if (Request->hasParam(F("password"), true))
            {
                ...
                if (Password_To_Check == Device_Password)
                {
                    for (uint8_t i = 0; i < MAXIMUM_LOGGED_CLIENTS; i++)
                    {
                        if (Logged_Clients_IP_Adress[i] == 0)
                        {
                            Logged_Clients_IP_Adress[i] == Request->client()->remoteIP();
                        }
                    }
